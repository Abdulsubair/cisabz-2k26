...
export const AdminPortal: React.FC<AdminPortalProps> = ({ onBackToWebsite }) => {
  ...
  const [showFinanceNotesModal, setShowFinanceNotesModal] = useState<boolean>(false);
  const [newNoteText, setNewNoteText] = useState<string>('');
  const [newNoteAmount, setNewNoteAmount] = useState<string>('');
  const [newNoteRecipient, setNewNoteRecipient] = useState<string>('');
  ...
  useEffect(() => {
    ...
    const unsubNotes = subscribeFinanceNotes((notes) => {
      setFinanceNotes(notes);
    });
  ...
  // Cash vs GPay breakdowns for current scoped section
  const currentPaidRecords = currentSectionRecords.filter((r) => r.status === 'PAID');

  const totalCashCollected = currentPaidRecords
    .filter((r) => r.paymentMode === 'CASH')
    .reduce((sum, r) => sum + (r.paidAmount || r.feeAmount), 0);
  const totalCashCount = currentPaidRecords.filter((r) => r.paymentMode === 'CASH').length;

  const totalGpayCollected = currentPaidRecords
    .filter((r) => r.paymentMode !== 'CASH')
    .reduce((sum, r) => sum + (r.paidAmount || r.feeAmount), 0);
  const totalGpayCount = currentPaidRecords.filter((r) => r.paymentMode !== 'CASH').length;

  // Date-wise breakdown of payments for current section (or overall)
  const dateWiseBreakdown = React.useMemo(() => {
    const map: Record<
      string,
      {
        dateKey: string;
    ...
    currentPaidRecords.forEach((r) => {
      const rawIso = r.paidAt || r.createdAt;
      let dateKey = 'Undated';
      ...
      const amt = r.paidAmount || r.feeAmount || 0;
      if (r.paymentMode === 'CASH') {
        map[dateKey].cashAmount += amt;
        map[dateKey].cashCount += 1;
      } else {
        map[dateKey].gpayAmount += amt;
        map[dateKey].gpayCount += 1;
      }
      map[dateKey].totalAmount += amt;
      map[dateKey].totalCount += 1;
      if (r.section) map[dateKey].sections.add(r.section);
    });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row">
              ...
              <>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-emerald-950/80 via-slate-900 to-teal-950/80 border border-emerald-500/40 p-6 rounded-3xl backdrop-blur-xl shadow-[0_0_40px_rgba(16,185,129,0.15)]">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                      <StickyNote className="w-4 h-4 text-purple-200" />
                      <span>Finance Notes</span>
                      {financeNotes.length > 0 && (
                        <span className="px-1.5 py-0.5 rounded-full bg-purple-950 text-purple-300 text-[10px] font-black border border-purple-400/40">
                          {financeNotes.length}
                        </span>
                      )}
                    </button>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5">
                  {/* Total Collected */}
                  <div className="bg-slate-900/90 border border-emerald-500/40 p-4 sm:p-4.5 rounded-2xl shadow-lg relative overflow-hidden flex flex-col justify-between">
                    ...
                    <span className="text-[10px] font-mono uppercase tracking-widest text-emerald-400 font-bold block mb-1">
                      {financeSelectedSection === 'ALL' ? 'Total Collected' : `${financeSelectedSection}`}
                    </span>
                    <span className="text-xl sm:text-3xl font-black font-orbitron text-emerald-400">
                      ₹{totalFinanceCollected.toLocaleString()}
                    </span>
                    <span className="text-[10px] font-mono text-slate-400 block mt-1">
                      {totalPaidFinanceCount} Paid ({overallCollectionPct}%)
                    </span>
                  </div>

                  {/* Cash Collected */}
                  <div className="bg-slate-900/90 border border-emerald-500/30 p-4 sm:p-4.5 rounded-2xl shadow-lg flex flex-col justify-between">
                    <span className="text-[10px] font-mono uppercase tracking-widest text-emerald-300 font-bold block mb-1 flex items-center gap-1">
                      <Wallet className="w-3.5 h-3.5 text-emerald-400" />
                      Cash Collected
                    </span>
                    <span className="text-xl sm:text-3xl font-black font-orbitron text-emerald-300">
                      ₹{totalCashCollected.toLocaleString()}
                    </span>
                    <span className="text-[10px] font-mono text-slate-400 block mt-1">
                      💵 {totalCashCount} Cash Payments
                    </span>
                  </div>

                  {/* GPay Collected */}
                  <div className="bg-slate-900/90 border border-cyan-500/30 p-4 sm:p-4.5 rounded-2xl shadow-lg flex flex-col justify-between">
                    <span className="text-[10px] font-mono uppercase tracking-widest text-cyan-300 font-bold block mb-1 flex items-center gap-1">
                      <Smartphone className="w-3.5 h-3.5 text-cyan-400" />
                      GPay / Online
                    </span>
                    <span className="text-xl sm:text-3xl font-black font-orbitron text-cyan-300">
                      ₹{totalGpayCollected.toLocaleString()}
                    </span>
                    <span className="text-[10px] font-mono text-slate-400 block mt-1">
                      📱 {totalGpayCount} GPay Payments
                    </span>
                  </div>

                  {/* Target Estimation */}
                  <div className="bg-slate-900/90 border border-slate-700/60 p-4 sm:p-4.5 rounded-2xl shadow-lg flex flex-col justify-between">
                    <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400 font-bold block mb-1">
                      {financeSelectedSection === 'ALL' ? 'Total Target' : 'Target'}
                    </span>
                    <span className="text-xl sm:text-3xl font-black font-orbitron text-white">
                      ₹{totalFinanceTarget.toLocaleString()}
                    </span>
                    <span className="text-[10px] font-mono text-slate-400 block mt-1">
                      {currentSectionRecords.length} Enrolled
                    </span>
                  </div>

                  {/* Pending Unpaid */}
                  <div className="bg-slate-900/90 border border-amber-500/40 p-4 sm:p-4.5 rounded-2xl shadow-lg flex flex-col justify-between">
                    <span className="text-[10px] font-mono uppercase tracking-widest text-amber-400 font-bold block mb-1">
                      Pending Unpaid
                    </span>
                    <span className="text-xl sm:text-3xl font-black font-orbitron text-amber-400">
                      {totalUnpaidFinanceCount}
                    </span>
                    <span className="text-[10px] font-mono text-slate-400 block mt-1">
                      ₹{(totalFinanceTarget - totalFinanceCollected).toLocaleString()} Left
                    </span>
                  </div>
                </div>

                {/* DAILY DATE-WISE CASH & GPAY BREAKDOWN CARD */}
                <div className="bg-slate-900/90 border border-emerald-500/30 p-5 rounded-3xl backdrop-blur-xl shadow-xl space-y-4">
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <h3 className="text-sm font-bold font-orbitron text-white flex items-center gap-2">
                          <span>Daily Collection Breakdown (Cash vs GPay)</span>
                          <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-mono font-bold border border-emerald-500/30">
                            {financeSelectedSection === 'ALL' ? 'All Classes Overall' : financeSelectedSection}
                          </span>
                        </h3>
                        <p className="text-[11px] font-mono text-slate-400">
                          Date-wise breakdown of Cash vs GPay collections for {financeSelectedSection === 'ALL' ? 'all sections combined' : financeSelectedSection}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-center gap-3 font-mono text-xs flex-wrap">
                      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-950/60 border border-emerald-500/30 text-emerald-300">
                        <Wallet className="w-3.5 h-3.5 text-emerald-400" />
                        <span>Cash: <strong>₹{totalCashCollected.toLocaleString()}</strong> ({totalCashCount})</span>
                      </div>
                      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-cyan-950/60 border border-cyan-500/30 text-cyan-300">
                        <Smartphone className="w-3.5 h-3.5 text-cyan-400" />
                        <span>GPay: <strong>₹{totalGpayCollected.toLocaleString()}</strong> ({totalGpayCount})</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {dateWiseBreakdown.length === 0 ? (
                    <div className="p-6 text-center text-slate-500 font-mono text-xs">
                      No paid transactions logged yet for {financeSelectedSection === 'ALL' ? 'any section' : financeSelectedSection}.
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {dateWiseBreakdown.map((item) => (
                        <div
                          key={item.dateKey}
                          className="bg-slate-950/80 border border-slate-800 hover:border-emerald-500/40 p-3.5 rounded-2xl transition-all space-y-2.5"
                        >
                          ...
                          <div className="grid grid-cols-2 gap-2 text-[11px] font-mono">
                            <div className="bg-slate-900 p-2 rounded-xl border border-emerald-500/20 flex flex-col justify-between">
                              <span className="text-slate-400 text-[10px] flex items-center gap-1">
                                <Wallet className="w-3 h-3 text-emerald-400" />
                                Cash Collection
                              </span>
                              <span className="text-emerald-300 font-bold text-xs mt-1">
                                ₹{item.cashAmount.toLocaleString()}
                              </span>
                              <span className="text-[9px] text-slate-500">{item.cashCount} student{item.cashCount !== 1 ? 's' : ''}</span>
                            </div>

                            <div className="bg-slate-900 p-2 rounded-xl border border-cyan-500/20 flex flex-col justify-between">
                              <span className="text-slate-400 text-[10px] flex items-center gap-1">
                                <Smartphone className="w-3 h-3 text-cyan-400" />
                                GPay Collection
                              </span>
                              <span className="text-cyan-300 font-bold text-xs mt-1">
                                ₹{item.gpayAmount.toLocaleString()}
                              </span>
                              <span className="text-[9px] text-slate-500">{item.gpayCount} student{item.gpayCount !== 1 ? 's' : ''}</span>
                            </div>
                          </div>

                          {financeSelectedSection === 'ALL' && item.sections.size > 0 && (
                            <div className="flex flex-wrap items-center gap-1 pt-1">
                              <span className="text-[9px] font-mono text-slate-500">Classes:</span>
                              {Array.from(item.sections).map((sec) => (
                                <span
                                  key={sec}
      ...
      {/* FINANCE NOTES MODAL */}
      {showFinanceNotesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl overflow-y-auto">
          <div className="relative w-full max-w-2xl bg-slate-900 border border-purple-500/40 rounded-3xl p-6 sm:p-8 shadow-[0_0_60px_rgba(147,51,234,0.25)] space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-2.5 text-purple-400 font-mono text-sm font-bold uppercase tracking-wider">
                <StickyNote className="w-5 h-5 text-purple-400" />
                <span>Symposium Finance Notes & Logbook</span>
              </div>
              <button
                onClick={() => setShowFinanceNotesModal(false)}
            ...
            {/* ADD NOTE FORM */}
            <form onSubmit={handleSaveNoteSubmit} className="bg-slate-950/80 border border-purple-500/30 p-4 sm:p-5 rounded-2xl space-y-4">
              <span className="text-xs font-mono font-bold uppercase text-purple-300 tracking-wider flex items-center gap-1.5">
                <Plus className="w-4 h-4 text-purple-400" />
                Write New Note / Finance Information
              </span>
              ...
              <div>
                <textarea
                  ...
                  placeholder="Enter main finance information, updates, cash receipts, expenditure notes..."
                  className="w-full p-3 rounded-xl bg-slate-900 border border-slate-800 text-white text-xs font-mono placeholder-slate-500 focus:outline-none focus:border-purple-500 transition-colors"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs font-mono">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-white text-xs focus:outline-none focus:border-purple-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-[10px] text-slate-400 uppercase font-bold mb-1">
                    Tag / Recipient (Optional)
                  </label>
                  <input
                    type="text"
                    value={newNoteRecipient}
                    onChange={(e) => setNewNoteRecipient(e.target.value)}
                    placeholder="e.g. Cashier / Snacks"
                    className="w-full p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-white text-xs focus:outline-none focus:border-purple-500"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:brightness-110 text-white font-mono font-bold text-xs uppercase tracking-wider transition-all shadow-[0_0_15px_rgba(147,51,234,0.3)] flex items-center gap-2 cursor-pointer disabled:opacity-50"
                >
                  <Check className="w-4 h-4" />
            ...
            {/* SEARCH NOTES & NOTES LIST */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono text-slate-400 font-bold uppercase tracking-wider">
                  Saved Logbook Notes ({financeNotes.length})
                </span>
                ...
                {financeNotes.length > 0 && (
                  <div className="relative w-48">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 rounded-lg bg-slate-950 border border-slate-800 text-white text-[11px] font-mono focus:outline-none focus:border-purple-500"
                    />
                  </div>
                )}
              </div>
              ...
              {financeNotes.length === 0 ? (
                <div className="p-8 text-center text-slate-500 font-mono text-xs bg-slate-950/60 border border-slate-800/80 rounded-2xl space-y-2">
                  <StickyNote className="w-8 h-8 text-slate-600 mx-auto" />
                  <p>No finance notes saved yet.</p>
                  <p className="text-[11px] text-slate-600">Write your first note above to save important notes, reminders, or cash logs!</p>
                </div>
              ) : (
                <div className="max-h-80 overflow-y-auto space-y-2.5 pr-1">
                  {financeNotes
                    ...
                    .map((noteItem) => (
                      <div
                        key={noteItem.id}
                        className="p-4 rounded-2xl bg-slate-950 border border-slate-800 hover:border-purple-500/40 transition-all space-y-2 relative group"
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <button
                            ...
                            className="text-slate-500 hover:text-rose-400 p-1 rounded-lg hover:bg-slate-900 transition-colors shrink-0 cursor-pointer"
                            title="Delete Note"
                          >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-900 text-[10px] font-mono text-slate-400">
                          <div className="flex items-center gap-2">
                            <span className="flex items-center gap-1 text-purple-300 font-bold bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20">
                              <Calendar className="w-3 h-3 text-purple-400" />
                              {noteItem.date}
                            </span>
                            {noteItem.amount !== undefined && (
                              <span className="font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                                ₹{noteItem.amount.toLocaleString()}
                              </span>
                            )}
                            {noteItem.recipient && (
                          ...
                          <span className="text-slate-500">
                            By {noteItem.createdBy || 'Admin'} • {new Date(noteItem.createdAt).toLocaleDateString()}
                          </span>
                        </div>
                      </div>
                    ))}
